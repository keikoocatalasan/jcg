...
Prepared: 2026-09-07. Status: v1.0.5 is published; physical-device acceptance
and landing-service redeploy confirmation remain open. v1.0.5 is now the
...
asset and is deployed through Render deployment dep-dago7nh5efls73bddiqg.
...
- [x] Landing page light theme, current previews, privacy/terms pages and
  ...
  dep-dago7nh5efls73bddiqg.
- [x] The Render ARM64 mirror now returns the exact published v1.0.4 asset:
...
- [x] Local QA debug coverage exercised camera permission, preview, bounded
  live analysis, shutter capture, sample recognition and portion confirmation;
  meal, water and weight edits persisted locally and queued sync operations.
- [ ] Production exact-APK camera/upload recognition and server-backed
  date-scoped persistence still require an authenticated physical-device flow.
